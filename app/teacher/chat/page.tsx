...
export default function TeacherChatPage() {
  ...
  useEffect(() => {
    async function loadChats() {
      ...
      try {
        const res = await fetch(`/api/teacher/chat?email=${session.user.email}`, { cache: "no-store" });
  ...
  async function sendMessage(e: React.FormEvent) {
    ...
    setMessages((prev) => [...prev, { id: tempId, sender: "teacher", text, createdAt: new Date().toISOString() }]);
    setInput("");
    setTimeout(() => bottomRef.current?.scrollIntoView({ behavior: "smooth" }), 10);
